      ...
      <Container>
        <ContentWithPaddingXl>
          ...
          <Text>
            ...
            <ul>
              <li>
                void turnToGreyscale(uintptr_t, unsigned int)<ul>
                <li>
                ...
                <li>
                  Expected Outputs and Assumptions<ul>
                  <li>
...
{/* <li>	void applyFilter(uintptr_t, unsigned int, unsigned int, bool, unsigned long)
<li>Function Description
<li>This function is used to apply convolution filters onto an image that is passed in via an image pointer. Using the images width and height, this method loops through each pixel and applies each filter that is marked active. The method will loop through each function and only apply the filters (from the class’s filter list) whose index is marked active (see the filterApplied argument for details). In addition, this also takes into account greyscale and RGB images, and will not perform extra loops through the RGB channels if the image is not using color. This method creates a temporary image array equal in size to the that of the passed-in image and stores the manipulated data from the filters. At the end, the function loops through this temporary image and transfers the new values over to the image data.
<li>Arguments
<li>Uintptr_t imagePtr (required)
<li>	An unsigned integer type that is capable of storing a data pointer, meaning it has the same size as a typical pointer. This is used to point to the beginning of an image in order to perform the selected convolutions on each pixel. Uintptr_t is used instead another pointer since Embind does not allow exposure of pointers directly.
<li>Unsigned int width (required)
<li>	A positive integer that represents the number of columns of pixels in the image (e.g., 400x300). This is used to keep track of column number in each row that is currently being manipulated by the convolution filter.
<li>Unsigned int height (required)
<li>	A positive integer that represents the number of rows of pixels in the image (e.g., 400x300). This is used to keep track of row number that is currently being manipulated by the convolution filter.
<li>Bool useColor (required)
<li>	A Boolean value (true or false) that is used to determine whether color is being used in the image currently. If this is true, then all three channels (RGB) will be looked at by the convolution filter and updated accordingly. If this is false, only the first will be looked at, and all three RGB will be set to the same thing since this assumes the image is in greyscale.
<li>Unsigned long filterApplied (optional)
<li>	A positive integer value that stores which filters are going to be used in the function. The way this is used, is each binary digit corresponds to a filter (e.g., 0x000001 is the first filter and 0x010010 is the second and fifth filters). This is used to determine which filters are active by ORing its binary with a 1 in the binary digit that corresponds to the filter being processed (e.g., filter 1 is “Gaussian Blur 3x3” so the value will be ORed with 0x1, and filter 5 is “Edge Detection” so the value will be ORed with 0x10000). By default, this value is 0x000000 so no filters will be active.
<li>Expected Output and Assumptions
<li>Void  although the return type is void, this function should modify the RGB values for each pixel in the image according to the active filters to display a new convoluted image.
<li>Inside this function, the uintptr_t is cast to a uint8_t pointer which is a pointer to an unsigned 8-bit integer array. This is done since the image data being passed in should contain data of the same type, which should be unsigned 8-bit integers. This is then updated with the convoluted values which should update the actual data passed-in.
<li>After this function is called, the image or canvas element should be reloaded, or new image data should be sent to the 2d context in order to show the updated image potentially now convoluted. This also should be called multiple times in succession if using a webcam feed to show the filter’s affect constantly.
<li>Although filterApplied has a default value, since this may be called repeatedly to actively show convolutions on an image, passing-in a 0 value long should be done.
<li>Before using this method with useColor set to false, the turnToGreyscale method should be utilized so that when only one channel is manipulated in the function, the correct effect will occur. If this method is not used or a greyscale image is not used, then the result of the convolution filters may not look correctly.
